...
window.showNotification = function (options) {
  ...
  const $el = NotificationAPI.$(`
    <div class="notification notification-${config.type}" id="${id}"
      ...
      <span class="icon icon-${config.icon} icon-16" aria-hidden="true"></span>
...
window.notifySuccess = (message, duration = 3000, position = 'top-right') =>
  showNotification({ type: 'success', message, icon: 'check', duration, position });

window.notifyError = (message, duration = 3000, position = 'top-right') =>
  showNotification({ type: 'error', message, icon: 'alert', duration, position });

window.notifyWarning = (message, duration = 3000, position = 'top-right') =>
  showNotification({ type: 'warning', message, icon: 'alert', duration, position });

window.notifyInfo = (message, duration = 3000, position = 'top-right') =>
  showNotification({ type: 'info', message, icon: 'info', duration, position });
